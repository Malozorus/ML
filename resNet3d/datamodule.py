...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        video = self.load_video_with_opencv(video_path)

        # S'assurer que la vidéo est sur le GPU si disponible
        
        video = video.to(torch.device("cuda"))
    ...
    def load_video_with_opencv(self, video_path):
...
class VideoDataModule(pytorch_lightning.LightningDataModule):
    ...
    _BATCH_SIZE = 8
    _NUM_WORKERS = 14  # Nombre de processus parallèles récupérant les données
...
def uniform_frame_sampling(video_tensor, target_frames=5):
    """
...
def video_resize_and_sample(video_tensor, size=(8, 16), T=5):
    # Resize
